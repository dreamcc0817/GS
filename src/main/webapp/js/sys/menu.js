$package('dreamcc.sysMenu');

dreamcc.sysMenu = function(){
	var _box = null;
	var _this = {
		...
		addDefBtns:function(){
			var defaultBtns= [
				{"btnName":"添加","menuid":2,"actionUrls":"save","btnType":"add"},
				{"btnName":"修改","menuid":2,"actionUrls":"getId|save","btnType":"edit"},
				{"btnName":"删除","menuid":2,"actionUrls":"delete","btnType":"remove"}
			];
		...
		addLine: function(data){
			...
			$(".remove-btn",line).click(function(){
				dreamcc.confirm('确认','你确定要删除该条记录吗?',function(r){
					if(r){
						_this.delLine(line);
					}
				})
  			...
  			dataGrid:{
  				title:'Menu List',
	   			...
	   			columns:[[
					...
					{field:'name',title:'菜单',width:120,sortable:true},
					...
					{field:'childMenus',title:'子菜单',width:120,align:'center',formatter:function(value,row,index){
						var html ="<a href='#' onclick='dreamcc.sysMenu.toList("+row.id+")'>子菜单管理("+row.subCount+")</a>";
						return html;
					}}
				]],
				toolbar:[
		...
		init:function(){
			...
			$('#delAllLine_btn').click(function(){
				dreamcc.confirm('Confirm','Are you sure you want to delete record?',function(r){
