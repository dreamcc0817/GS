var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					dreamcc.getById(Action.getId,data,function(result){
					...
					$.messager.confirm('确认','确认要删除该条记录吗?',function(r){  
					    if (r){
					    	...
					    	var  data = $("input[name='"+idKey+"']", Form.list ).fieldSerialize(); //序列化字段
					   		dreamcc.deleteForm(Action.remove,data,function(result){
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
									callback(result);
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					dreamcc.saveForm(Form.edit,function(data){
		...
		var Utils = {
			...
			checkSelect : function(rows){//检查grid是否有勾选的行, 有返回 true,没有返回false
				...
				dreamcc.alert('警告','未选中记录.','warning');  
				return false;
			...
			checkSelectOne : function(rows){//检查grid是否只勾选了一行,是返回 true,否返回false
				...
				dreamcc.alert('警告','只能选择一行记录.','warning');  
				return false;
			}
		}
